...
class ForgotPasswordRequestCubit extends Cubit<ForgotPasswordRequestState> {
  ...
  Future<void> requestforgotPassword({required String email, required String password, required String schoolCode}) async {
    emit(ForgotPasswordRequestInProgress());
    try {
      await _authRepository.forgotPassword(email: email, schoolCode: schoolCode, password: password);
